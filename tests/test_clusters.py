...
def row(i, signature="discount not applied", *, severity="medium", status="usable", confidence=0.5,
        ungradable=False, created="2026-09-10T08:00:0{}Z"):
    return {
        "feedback_id": f"fb-{i}", "trace_id": f"trace-{i}", "deployment_id": 3,
        "failure_signature": signature, "failure_summary": f"summary {i}",
        "severity": severity, "correction_status": status, "confidence": confidence,
        "ungradable": ungradable, "created_at": created.format(i),
    }
...
class TestAssigning:
    def test_the_same_signature_is_one_cluster_and_rows_point_at_it(self):
        ...
        assert rows[0]["cluster_id"] == rows[1]["cluster_id"] == by_sig["discount not applied"].cluster_id
        assert rows[2]["cluster_id"] == by_sig["wrong tool"].cluster_id
        assert by_sig["discount not applied"].first_seen == datetime(2026, 9, 10, 8, 0, 0, tzinfo=timezone.utc)
        assert by_sig["discount not applied"].last_seen == datetime(2026, 9, 10, 8, 0, 1, tzinfo=timezone.utc)

    def test_an_existing_cluster_grows_instead_of_being_duplicated(self):
        existing = c.Cluster(cluster_id="c-old", deployment_id=3, signature="discount not applied", size=9,
                             severity="low", representative_feedback_id="fb-old",
    ...
    def test_the_representative_is_the_best_member_and_only_moves_for_a_better_one(self):
        rows = [row(0, severity="low", status="missing", confidence=0.9),
        ...
        existing = c.from_api({"clusterId": "c", "deploymentId": 3, "signature": "s", "size": 3,
                               "severity": "high", "representativeFeedbackId": "fb-rep"})
        c.assign_clusters([row(7, "s", severity="high", status="missing")], [existing])
        assert existing.representative_feedback_id == "fb-rep"
        c.assign_clusters([row(8, "s", severity="critical", status="usable")], [existing])
    ...
    def test_a_dismissed_cluster_counts_its_new_members_but_stays_dismissed(self):
        existing = c.Cluster(cluster_id="c", deployment_id=3, signature="s", size=1, status="dismissed")
...
class TestKnownSignatures:
    def test_largest_live_clusters_first_without_the_dismissed(self):
        clusters = [c.Cluster("a", 3, "small", size=1), c.Cluster("b", 3, "big", size=40),
                    c.Cluster("d", 3, "gone", size=100, status="dismissed"), c.Cluster("e", 3, "big", size=2)]
...
class TestRows:
    def test_a_row_has_every_column_and_no_null_timestamp(self):
        ...
        assert fresh["first_seen"] == now and fresh["last_seen"] == now and fresh["updated_at"] == now
        assert fresh["decided_at"] == "" and fresh["status"] == "open"
        assert set(fresh) >= {"cluster_id", "deployment_id", "signature", "label", "severity", "size",
                              "representative_trace_id", "representative_feedback_id", "promoted_task_id",
                              "dismiss_reason", "decided_by"}

    def test_api_shape_round_trips(self):
        parsed = c.from_api({"clusterId": "c", "deploymentId": "3", "signature": "s", "size": "4",
                             "firstSeen": "2026-09-01T00:00:00Z", "lastSeen": 1_789_000_000_000,
                             "status": "promoted", "promotedTaskId": "t"})
