...
class TestRunVerifier:
    ...
    def test_accepts_when_either_side_does_not_know_the_deployment(self, monkeypatch):
        monkeypatch.setenv("DEPLOYMENT_ID", "12")
        assert RunVerifier(lambda run_id: {"runId": run_id}).verify(EvalTrial(run_id="r"))
        monkeypatch.delenv("DEPLOYMENT_ID")
        assert RunVerifier(lambda run_id: {"deploymentId": 99}).verify(EvalTrial(run_id="r"))
...
class TestManifest:
    def test_declares_per_request_when_asked(self, no_env_eval_mode):
        assert TestClient(build_app()).get("/.well-known/hopsworks-agent.json").json()[
            "capabilities"
        ]["eval_per_request"] is False
        manifest = TestClient(build_app(eval_per_request=True)).get(
            "/.well-known/hopsworks-agent.json"
        ).json()
...
class TestPerRequest:
    ...
    def test_baggage_is_ignored_by_an_agent_that_did_not_declare(self, no_env_eval_mode):
    ...
    def test_environment_wins_and_skips_verification(self, monkeypatch):
        ...
        assert reply(chat(client, {"baggage": BAGGAGE})) == (
            "tool=True run=run-1"
        )
