...
class TestReadingADirectory:
    ...
    def test_the_entry_is_found_under_a_hopsfs_or_git_relative_name(self):
        ...
        assert src._resolve_entry(files, "chinook/support_agent.py") == "chinook/support_agent.py"
        assert src._resolve_entry(files, "/Projects/g1/Models/agent/1/Files/chinook/support_agent.py") == \
            "chinook/support_agent.py"
        assert src._resolve_entry(files, "support_agent.py") == "chinook/support_agent.py"
...
class TestWhereTheCodeIs:
    def test_comes_off_the_serving_view(self):
        location = src.CodeLocation.from_serving({
            "gitUrl": "https://github.com/o/r", "gitBranch": "main", "gitResolvedBranch": "release",
            "gitCurrentCommit": "abc123", "predictor": "chinook/support_agent.py", "modelPath": None,
        })
...
class TestCloning:
    def test_fetches_the_running_commit_shallowly(self, tmp_path, monkeypatch):
        ...
        target = src.clone_repository("https://github.com/o/r", "main", "abc123", str(tmp_path), run=run)
    ...
    def test_an_unreadable_location_is_an_empty_bundle_with_the_reason(self, tmp_path, monkeypatch):
        ...
        result = src.load_agent_source(src.CodeLocation(git_url="https://github.com/o/r"), workdir=str(tmp_path))
        assert not result
...
class TestChoosingFiles:
    ...
    def test_files_that_mention_the_tools_called_or_the_failure_are_added(self):
        chosen = [path for path, _ in src.relevant_files(bundle(), tool_names=["recall_interests"])]
        assert "README.md" in chosen
        chosen = [path for path, _ in src.relevant_files(bundle(), clues=["noop returned nothing"])]
        assert "agent/unrelated.py" in chosen

    def test_the_budget_clips_and_says_so(self):
        files = {"a.py": "x" * 500, "b.py": "y" * 500}
        chosen = src.relevant_files(src.SourceBundle(files=files, entry="a.py"), clues=["yyyy"], budget_chars=800)
    ...
    def test_rendering_numbers_the_lines_under_each_path(self):
        text = src.render_source([("agent/tools.py", "def f():\n    pass\n")], origin="git x@main")
...
class TestImports:
    def test_resolves_relative_and_absolute_imports_inside_the_repository(self):
        files = bundle().files
        assert src.imported_files(files, "agent/main.py") == ["agent/main.py", "agent/tools.py", "agent/memory.py"]
    ...
    def test_module_candidates(self, module, expected):
        assert src._module_candidates(bundle().files, "agent/main.py", module) == expected
