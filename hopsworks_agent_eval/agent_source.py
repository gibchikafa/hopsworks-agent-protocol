...
SOURCE_SUFFIXES = (".py", ".md", ".txt", ".toml", ".yaml", ".yml", ".json", ".cfg", ".ini", ".sql")
#: Directories nobody's agent lives in.
SKIPPED_DIRS = {".git", "__pycache__", "node_modules", ".venv", "venv", "dist", "build", ".mypy_cache",
...
class CodeLocation:
    ...
    def from_serving(cls, view: dict[str, Any] | None) -> "CodeLocation":
        ...
        return cls(
            ...
            git_branch=str(view.get("gitResolvedBranch") or view.get("gitBranch") or "").strip(),
...
def bundle_from_directory(root: str | os.PathLike[str], *, entry: str = "", origin: str = "") -> SourceBundle:
    ...
    for path in sorted(p for p in root_path.rglob("*") if p.is_file()):
        ...
        files[relative] = text
        total += size
    return SourceBundle(files=files, entry=_resolve_entry(files, entry), origin=origin, skipped=skipped)
...
def _resolve_entry(files: dict[str, str], entry: str) -> str:
    ...
    for path in files:
        if candidate.endswith("/" + path) or path.endswith("/" + candidate) or Path(path).name == Path(candidate).name:
            return path
    return ""
    ...
    while pending:
        ...
        for item in listing.get("items") or []:
            ...
            if attributes.get("dir"):
                ...
                continue
            relative = item_path[len(root):].lstrip("/") if item_path.startswith(root) else Path(item_path).name
            if not _is_source(relative) or int(attributes.get("size") or 0) > MAX_FILE_BYTES:
                continue
...
def load_agent_source(location: CodeLocation, *, dataset_api: Any = None,
                      workdir: str | None = None) -> SourceBundle:
    ...
    if not location.known():
        log.info("the deployment's code location is not known; reviewing without source")
    ...
    try:
        if location.git_url:
            ...
            root = clone_repository(location.git_url, location.git_branch, location.git_commit, into)
            bundle = bundle_from_directory(root, entry=location.script_file,
                                           origin=f"git {location.git_url}@{ref}")
        else:
            ...
            bundle = bundle_from_directory(root, entry=location.script_file,
                                           origin=f"hopsfs {location.model_path}")
        log.info("read %d source files from %s (entry %s)", len(bundle.files), bundle.origin,
                 bundle.entry or "unknown")
        return bundle
    except Exception as err:  # noqa: BLE001 -- said in the log, not thrown at the run
        log.warning("could not read the agent's source from %s: %s",
                    location.git_url or location.model_path, err)
...
_IMPORT = re.compile(r"^\s*(?:from\s+([.\w]+)\s+import|import\s+([\w.]+(?:\s*,\s*[\w.]+)*))", re.MULTILINE)
...
def imported_files(files: dict[str, str], entry: str, limit: int = 40) -> list[str]:
    ...
    while queue and len(ordered) < limit:
        ...
        for match in _IMPORT.finditer(files[current]):
            modules = [match.group(1)] if match.group(1) else [m.strip() for m in match.group(2).split(",")]
...
def relevant_files(bundle: SourceBundle, *, tool_names: Sequence[str] = (), clues: Sequence[str] = (),
                   budget_chars: int = DEFAULT_SOURCE_CHARS) -> list[tuple[str, str]]:
    ...
    if not ordered and not mentioned:
        ...
        mentioned = sorted((p for p in rest if p.endswith(".py")), key=lambda p: (len(bundle.files[p]), p))
...
def render_source(files: Sequence[tuple[str, str]], origin: str = "") -> str:
    ...
    for path, text in files:
        numbered = "\n".join(f"{n:4d}  {line}" for n, line in enumerate(text.splitlines(), start=1))
