...
CATEGORIES = ("wrong_answer", "missing_info", "wrong_tool", "refused", "unsafe", "other")
...
ASSERTION_KINDS = (
    ...
    "tool_called_before_answer", "matches_number",
)
...
def render_triage_prompt(inp: TriageInput, *, context_turns: int = 20) -> str:
    ...
    transcript = "\n\n".join(f"{who}: {text}" for who, text in turns if text) or "(no earlier turns)"
    tools = ""
...
def _text(parsed: dict[str, Any], key: str, limit: int, *, required: bool = False) -> str:
    raw = parsed.get(key)
...
def parse_triage(text: str) -> TriageResult:
    ...
    return TriageResult(
        ...
        failure_signature=_normalise_signature(_text(parsed, "failure_signature", _MAX_SIGNATURE, required=True)),
        ...
        proposed_expected_tool_behavior=_text(parsed, "proposed_expected_tool_behavior", _MAX_RUBRIC),
...
def triage(complete: Callable[[str], str], inp: TriageInput, *,
           context_turns: int = 20) -> tuple[TriageResult | None, str]:
...
def triage_row(feedback: dict[str, Any], result: TriageResult | None, *, run_id: str,
               provider: str, model: str, error: str = "",
    ...
    if result is not None:
        row.update({
            ...
            "proposed_assertions": json.dumps([asdict(a) for a in result.proposed_assertions]),
