...
class ApiModel:
    ...
    @classmethod
    def from_api(cls: type[T], data: dict[str, Any] | None) -> T:
        data = data or {}
        values: dict[str, Any] = {}
        for f in fields(cls):
...
class TraceSummary(ApiModel):
    ...
    def started_at(self) -> datetime | None:
        return as_datetime(self.start_time_ns / 1_000_000) if self.start_time_ns else None
...
class Trace(ApiModel):
    ...
    def root(self) -> dict[str, Any] | None:
        return next((s for s in self.spans if not s.get("parentSpanId")), self.spans[0] if self.spans else None)

    def attributes_of(self, span_id: str) -> dict[str, str]:
        return {a.get("attrKey", ""): a.get("attrValue", "") for a in self.span_attributes
    ...
    def tool_calls(self) -> list[dict[str, Any]]:
        ...
        for span in sorted(self.spans, key=lambda s: s.get("startTimeNs") or 0):
            ...
            if kind != "TOOL" and attrs.get("gen_ai.operation.name") != "execute_tool":
                continue
            calls.append({
                "name": attrs.get("tool.name") or attrs.get("gen_ai.tool.name") or span.get("name"),
                "arguments": attrs.get("input.value") or attrs.get("gen_ai.tool.call.arguments") or "",
                "result": attrs.get("output.value") or attrs.get("gen_ai.tool.call.result") or "",
