...
def _suite_ref(suite: Suite | str, version: int | None = None) -> tuple[str, int | None]:
...
def _messages(question: str | Sequence[str] | Sequence[dict[str, Any]]) -> tuple[str, str]:
    """A question, a list of user turns, or full messages, as the JSON a task stores."""
    if isinstance(question, str):
        messages: list[dict[str, Any]] = [{"role": "user", "content": question}]
    else:
        messages = [m if isinstance(m, dict) else {"role": "user", "content": str(m)} for m in question]
...
class Suites:
    ...
    def list(self) -> list[Suite]:
        return self._client._bind_all(Suite.list_from_api(self._http.get(f"{EVALS}/suites")))

    def get(self, suite_id: str, version: int | None = None) -> Suite:
        return self._client._bind(Suite.from_api(self._http.get(f"{EVALS}/suites/{suite_id}", version=version)))
    ...
    def create(self, name: str, *, checks: Sequence[dict[str, Any]] = (), description: str = "",
               tags: Sequence[str] = (), blocks_are_success: bool = False, run_on_update: bool = False,
               gate_metric: str = "", gate_threshold: float | None = None, pass_policy: str = "all",
               pass_threshold: float | None = None, execution_mode: str | None = None) -> Suite:
        ...
        return self._client._bind(Suite.from_api(self._http.post(f"{EVALS}/suites", body)))
    ...
    def update(self, suite: Suite | str, version: int | None = None, **changes: Any) -> Suite:
        ...
        for key, value in changes.items():
            ...
            body["".join(p.capitalize() if i else p for i, p in enumerate(key.split("_")))] = value
        return self._client._bind(Suite.from_api(self._http.put(f"{EVALS}/suites/{suite_id}", body,
                                                                 version=version)))

    def set_checks(self, suite: Suite | str, checks: Sequence[dict[str, Any]],
                   version: int | None = None) -> list[Check]:
        """Replace a draft's checks. Refused once published."""
        suite_id, version = _suite_ref(suite, version)
        rows = self._http.put(f"{EVALS}/suites/{suite_id}/evaluators", {"evaluators": list(checks)},
                              version=version)
    ...
    def publish(self, suite: Suite | str, version: int | None = None) -> Suite:
        ...
        suite_id, version = _suite_ref(suite, version)
        return self._client._bind(Suite.from_api(self._http.post(f"{EVALS}/suites/{suite_id}/publish",
                                                                  version=version)))

    def delete(self, suite: Suite | str, version: int | None = None, force: bool = False) -> None:
        suite_id, version = _suite_ref(suite, version)
        self._http.delete(f"{EVALS}/suites/{suite_id}", version=version, force="true" if force else None)

    def tasks(self, suite: Suite | str, version: int | None = None) -> list[Task]:
        suite_id, version = _suite_ref(suite, version)
        return self._client._bind_all(Task.list_from_api(self._http.get(f"{EVALS}/suites/{suite_id}/tasks",
                                                                          version=version)))

    def import_tasks(self, suite: Suite | str, tasks: Iterable[dict[str, Any]],
        ...
        for task in tasks:
            ...
            rows.append(row)
        return self._http.post(f"{EVALS}/suites/{suite_id}/tasks/import", rows, version=version) or {}
...
class Tasks:
    ...
    def list(self, *, unassigned: bool = False, pending_redaction: bool = False) -> list[Task]:
        """Every task, or the promotion queue: unassigned to a suite, or waiting for a redaction review."""
        return self._client._bind_all(Task.list_from_api(self._http.get(
            f"{EVALS}/tasks",
    ...
    def get(self, task_id: str) -> Task:
        return self._client._bind(Task.from_api(self._http.get(f"{EVALS}/tasks/{task_id}")))

    def create(self, question: str | Sequence[str] | Sequence[dict[str, Any]], *,
               expectations: dict[str, str] | None = None, category: str | None = None) -> Task:
        ...
        return self._client._bind(Task.from_api(self._http.post(f"{EVALS}/tasks", body)))

    def add_to_suite(self, task: Task | str, suite: Suite | str, version: int | None = None,
                     expectations: dict[str, str] | None = None) -> Task:
        """Join a task to a suite with what it expects of each of the suite's checks, by check name."""
        suite_id, version = _suite_ref(suite, version)
        return self._client._bind(Task.from_api(self._http.post(
            f"{EVALS}/tasks/{_task_id(task)}/suite", {"expectations": expectations or {}},
            suiteId=suite_id, version=version)))

    def remove_from_suite(self, task: Task | str) -> Task:
        return self._client._bind(Task.from_api(self._http.delete(f"{EVALS}/tasks/{_task_id(task)}/suite")))

    def delete(self, task: Task | str) -> None:
        self._http.delete(f"{EVALS}/tasks/{_task_id(task)}")

    def promote(self, deployment_id: int, trace_id: str, *, expectations: dict[str, str] | None = None,
        ...
        if category:
            body["category"] = category
        return self._client._bind(Task.from_api(self._http.post(
            f"{EVALS}/tasks/from-trace/{trace_id}", body, deploymentId=deployment_id)))

    def confirm_redaction(self, task: Task | str, *, input_messages: str | Sequence[dict[str, Any]] | None = None,
                          expectations: dict[str, str] | None = None) -> Task:
        """Record that a named person checked a promoted task for personal data, with any edits."""
        body: dict[str, Any] = {}
        if input_messages is not None:
            body["inputMessages"] = input_messages if isinstance(input_messages, str) else json.dumps(
                list(input_messages))
        if expectations is not None:
            body["expectations"] = expectations
        return self._client._bind(Task.from_api(self._http.post(f"{EVALS}/tasks/{_task_id(task)}/redaction", body)))

    def forget_trace(self, deployment_id: int, trace_id: str) -> None:
        """Remove every task promoted from a trace, for a deletion request."""
        self._http.delete(f"{EVALS}/tasks/from-trace/{trace_id}", deploymentId=deployment_id)

    def add_to_regressions(self, task: Task | str, *, input_messages: str | None = None) -> Task:
        ...
        return self._client._bind(Task.from_api(self._http.post(
            f"{EVALS}/tasks/{_task_id(task)}/regressions", body)))
...
class Evaluators:
    ...
    def save(self, name: str, checks: Sequence[dict[str, Any]], description: str = "") -> EvaluatorTemplate:
        """Save a named set of checks for reuse. A suite copies them in and never points back."""
        return EvaluatorTemplate.from_api(self._http.post(f"{EVALS}/evaluators", {
            "name": name, "description": description, "spec": json.dumps(list(checks)),
        }))

    def delete(self, template: EvaluatorTemplate | str) -> None:
        template_id = template.template_id if isinstance(template, EvaluatorTemplate) else template
    ...
    def install_defaults(self, overwrite: bool = False) -> list[str]:
        ...
        for template in default_templates():
            if template["name"] in existing and not overwrite:
                continue
            self.save(template["name"], json.loads(template["spec"]), template.get("description", ""))
    ...
    def judge_models(self, provider: str, secret: str | None = None) -> list[str]:
        ...
        return list(self._http.get(f"{EVALS}/judge-models", provider=provider, secret=secret) or [])
...
class Runs:
    ...
    def list(self, deployment_id: int | None = None) -> list[Run]:
        return self._client._bind_all(Run.list_from_api(self._http.get(f"{EVALS}/runs", deploymentId=deployment_id)))

    def get(self, run_id: str) -> Run:
        return self._client._bind(Run.from_api(self._http.get(f"{EVALS}/runs/{run_id}")))

    def start(self, suite: Suite | str, deployment_id: int, *, version: int | None = None,
              n_trials: int = 1, start: bool = True) -> Run:
        """Run a published suite against a deployment. ``n_trials`` above 1 is pass^k, not pass@k."""
        suite_id, version = _suite_ref(suite, version)
        return self._client._bind(Run.from_api(self._http.post(
            f"{EVALS}/runs", suiteId=suite_id, version=version, deploymentId=deployment_id,
            nTrials=n_trials, start="true" if start else "false")))

    def sample(self, deployment_id: int, *, evaluator: EvaluatorTemplate | str | None = None,
               suite: Suite | None = None, since: Any = None, until: Any = None, start: bool = True) -> Run:
        ...
        template_id = evaluator.template_id if isinstance(evaluator, EvaluatorTemplate) else evaluator
        return self._client._bind(Run.from_api(self._http.post(
            f"{EVALS}/sample-runs", deploymentId=deployment_id, templateId=template_id,
            suiteId=suite.suite_id if suite else None, suiteVersion=suite.version if suite else None,
            **{"from": epoch_ms(since), "to": epoch_ms(until)}, start="true" if start else "false")))

    def sample_watermark(self, deployment_id: int) -> int | None:
        """Where the next sample would start, as epoch milliseconds."""
        body = self._http.get(f"{EVALS}/sample-runs/watermark", deploymentId=deployment_id) or {}
        return body.get("from")

    def start_recorded(self, run: Run | str) -> Run:
        run_id = run.run_id if isinstance(run, Run) else run
        return self._client._bind(Run.from_api(self._http.post(f"{EVALS}/runs/{run_id}/start")))

    def trials(self, run: Run | str) -> list[Trial]:
        run_id = run.run_id if isinstance(run, Run) else run
        return Trial.list_from_api(self._http.get(f"{EVALS}/runs/{run_id}/trials"))

    def results(self, run: Run | str, trial: Trial | str | None = None) -> list[EvaluatorResult]:
        run_id = run.run_id if isinstance(run, Run) else run
        trial_id = trial.trial_id if isinstance(trial, Trial) else trial
        return EvaluatorResult.list_from_api(self._http.get(f"{EVALS}/runs/{run_id}/evaluator-results",
                                                            trialId=trial_id))
    ...
    def trend(self, deployment_id: int, limit: int = 20) -> list[RunMetric]:
        ...
        return RunMetric.list_from_api(self._http.get(f"{EVALS}/runs/metrics", deploymentId=deployment_id,
                                                      limit=limit))
    ...
    def wait(self, run: Run | str, *, timeout_s: float = 1800, poll_s: float = 5) -> Run:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise AgentEvalsError(f"run {run_id} still {current.status} after {timeout_s:.0f}s")
            time.sleep(poll_s)
...
class Jobs:
    ...
    def eval_job(self, deployment_id: int) -> EvalJob:
        ...
        return EvalJob.from_api(self._http.get(f"{EVALS}/runner-job", deploymentId=deployment_id))

    def eval_jobs(self, deployment_id: int) -> list[EvalJob]:
        return EvalJob.list_from_api(self._http.get(f"{EVALS}/eval-jobs", deploymentId=deployment_id))

    def ensure_eval_job(self, deployment_id: int, *, name: str | None = None, suites: Sequence[Suite | str] = (),
                        evaluators: Sequence[EvaluatorTemplate | str] = (), monitor: bool | None = None,
                        environment_name: str | None = None, cores: int | None = None,
                        memory: int | None = None, gpus: int | None = None) -> EvalJob:
        """Create the deployment's evaluation job unless it exists; an existing one is returned untouched."""
        body = {
            "name": name,
            "suites": [f"{s.suite_id}:{s.version}" if isinstance(s, Suite) else s for s in suites] or None,
            "evaluators": [e.template_id if isinstance(e, EvaluatorTemplate) else e for e in evaluators] or None,
            "monitor": monitor,
        ...
        return EvalJob.from_api(self._http.post(f"{EVALS}/runner-job", {k: v for k, v in body.items() if v is not None},
                                                deploymentId=deployment_id))

    def update_eval_job(self, name: str, *, suites: Sequence[Suite | str] | None = None,
                        evaluators: Sequence[EvaluatorTemplate | str] | None = None, monitor: bool | None = None,
                        environment_name: str | None = None, cores: int | None = None,
                        memory: int | None = None) -> EvalJob:
        body: dict[str, Any] = {}
        if suites is not None:
            body["suites"] = [f"{s.suite_id}:{s.version}" if isinstance(s, Suite) else s for s in suites]
        if evaluators is not None:
            body["evaluators"] = [e.template_id if isinstance(e, EvaluatorTemplate) else e for e in evaluators]
        for key, value in (("monitor", monitor), ("environmentName", environment_name), ("cores", cores),
                           ("memory", memory)):
    ...
    def run_eval_job(self, name: str) -> list[Run]:
        ...
        return self._client._bind_all(Run.list_from_api(self._http.post(f"{EVALS}/eval-jobs/{name}/run")))

    # ── failure analysis job ───────────────────────────────────────────────

    def review_jobs(self, deployment_id: int) -> list[ReviewJob]:
        return self._client._bind_all(ReviewJob.list_from_api(self._http.get(f"{EVALS}/review-jobs",
                                                                              deploymentId=deployment_id)))
    ...
    def review_job_defaults(self, deployment_id: int) -> ReviewJob:
        return ReviewJob.from_api(self._http.get(f"{EVALS}/review-jobs/defaults", deploymentId=deployment_id))
    ...
    def ensure_review_job(self, deployment_id: int, **settings: Any) -> ReviewJob:
        ...
        """
        return self._client._bind(ReviewJob.from_api(self._http.post(
            f"{EVALS}/review-jobs", _review_body(settings), deploymentId=deployment_id)))

    def update_review_job(self, job: ReviewJob | str, **settings: Any) -> ReviewJob:
        name = job.name if isinstance(job, ReviewJob) else job
        return self._client._bind(ReviewJob.from_api(self._http.put(
            f"{EVALS}/review-jobs/{name}", _review_body(settings))))

    def delete_review_job(self, job: ReviewJob | str) -> None:
        name = job.name if isinstance(job, ReviewJob) else job
        self._http.delete(f"{EVALS}/review-jobs/{name}")

    def analyse(self, job: ReviewJob | str, *, since: Any = None, until: Any = None,
                trace_id: str | None = None) -> Run:
        """Run the analysis now: since the last run by default, or over a window, or on one trace."""
        name = job.name if isinstance(job, ReviewJob) else job
        return self._client._bind(Run.from_api(self._http.post(
            f"{EVALS}/review-jobs/{name}/run", **{"from": epoch_ms(since), "to": epoch_ms(until)},
            traceId=trace_id)))
    ...
    def regressions(self, deployment_id: int) -> RegressionSuite:
        ...
        return RegressionSuite.from_api(self._http.get(f"{EVALS}/regressions", deploymentId=deployment_id))

    def run_regressions(self, deployment_id: int) -> Run:
        """Publish the draft regressions, if any, and run them against the deployment."""
        return self._client._bind(Run.from_api(self._http.post(f"{EVALS}/regressions/run",
                                                                deploymentId=deployment_id)))
