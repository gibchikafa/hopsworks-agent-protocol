...
class Cluster:
    ...
    representative_score: tuple = field(default_factory=tuple, repr=False, compare=False)
...
def from_api(entry: dict[str, Any]) -> Cluster:
    ...
    return Cluster(
        ...
        representative_score=(1, SEVERITY_RANK.get(str(entry.get("severity") or "low"), 0), 0.0),
    )


def known_signatures(clusters: Iterable[Cluster], limit: int = KNOWN_SIGNATURES_LIMIT) -> list[str]:
...
def assign_clusters(rows: Sequence[dict[str, Any]], existing: Iterable[Cluster],
    ...
    for row in rows:
        ...
        if SEVERITY_RANK.get(str(row.get("severity") or "low"), 0) > SEVERITY_RANK.get(cluster.severity, 0):
            cluster.severity = str(row.get("severity"))
        score = _score(row)
        if not cluster.representative_feedback_id or score > cluster.representative_score:
            ...
            cluster.label = str(row.get("failure_summary") or cluster.label or signature)
